...
socket.on("joinRoom", (data) =>{
	...
	socket.join(data.room, () => {
		console.log("joined room: " + data.room + "with id: " + data.mySocket);
		io.in(data.room).emit('testRoom', socket.id)
		console.log("joined room: " + data.room + "with id: " + data.mySocket);
		io.to(data.mySocket).emit('testRoom', socket.id)});
		console.log("joined room: " + data.room + "with id: " + data.mySocket);
...
socket.on('playerPickedCard', (data) => {
	...
	if(atadasiKor && cycle % 4 != 0){	
		...
		console.log(aktualisanAtadottKartyak)
